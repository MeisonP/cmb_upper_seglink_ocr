...
def draw_bbox(image_data, line, color):
    ...
    def draw_oriented_bbox():
        ...
        util.img.draw_contours(image_data, cnts, -1, color = color, border_width = 2)
...
def visualize(image_root, det_root, output_root, gt_root = None):
    ...
    image_names = util.io.ls(image_root, '.jpg')    # the png should also be included!
    for image_idx, image_name in enumerate(image_names):
        ...
        for line in det_lines:
            draw_bbox(det_image, line, color = util.img.COLOR_BGR_YELLOW)
