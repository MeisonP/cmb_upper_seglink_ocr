...
def create_clones(batch_queue):        
    ...
    for clone_idx, gpu in enumerate(config.gpus):
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=True):
            # the variables has been created in config.init_config
    ...
    if FLAGS.using_moving_average:
        ...
        with decay = {}'.format(FLAGS.moving_average_decay))
...
def train(train_op):
    ...
    slim.learning.train(
            ...
            # init_fn = init_fn,    # Initializing a model from a checkpoint
